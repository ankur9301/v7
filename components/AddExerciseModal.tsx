...
const AddExerciseModal: React.FC<AddExerciseModalProps> = ({
  ...
  const handleWorkoutPress = (workout: Workout) => {
    if (multipleSelection) {
      if (selectedWorkoutIds.includes(Number(workout.id))) {
        setSelectedWorkoutIds(selectedWorkoutIds.filter((id) => id !== workout.id));
      } else {
        setSelectedWorkoutIds([...selectedWorkoutIds, Number(workout.id)]);
      }
    } else {
      setSelectedWorkoutIds(
        selectedWorkoutIds.includes(Number(workout.id)) ? [] : [Number(workout.id)]
      );
    }
  };
  ...
  const handleDone = () => {
    const selectedWorkouts = workouts.filter((workout) =>
      selectedWorkoutIds.includes(Number(workout.id))
    );
